...
- **Migração 046 (`order_erp_sync`)** nasceu às 15:22 de 11/09, depois deste brief. É a "foto do que o Control conhece", para o botão *Atualizar no ERP* (venda interna editando pedido já lançado). Ela **já está no caminho da integração**: desde 14/09 `confirmOrderImport` chama `registrarNoErp` depois de gravar o número (`partner.service.ts`, fim da função). Sem a tabela a chamada devolve `sem_tabela` em silêncio. Arquivo pronto: `_tools/SQL-PARA-RODAR-046.sql`. **Medido em 15/09: já aplicada nos dois bancos.** Não existe 045 (número reservado por mensagem — não assumir livre).
- **Estado dos bancos, medido em 11/09 e 15/09:** colunas de 041, 043, 044 e a tabela da 046 existem nos DOIS bancos. **A 042 (índice único do número do Control) existe na PLUMENE e FALTA na Corpo Sensual** — conferido pelo Yan em 15/09 por `pg_indexes` no SQL Editor. Há 0 números repetidos, então colar o `CREATE UNIQUE INDEX` da 042 na Corpo Sensual é seguro. `SQL-PARA-RODAR-041-NA-PLUMENE.sql` está obsoleto.
...
3. **§7 passo 3 — está quase vazio** (ver 0.1). O que resta: colar a 042 na Corpo Sensual.
