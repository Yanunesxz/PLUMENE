...
10. **Cores são internas:** nada muda (continuam na observação do item, `cor` sempre `00001`). Desde 22/09/2026 o texto da observação traz o **número da bolinha do catálogo** (`"Cor 2"`, `"3M Cor 2 / 2G Cor 1"`) no lugar do nome; cor sortida e cor única continuam pelo nome (`"Variadas"`, `"Cor única"`) — pedido do Yan para o estoque; o app continua mostrando o nome.
...
### 0.8 Edição do cadastro do cliente no app (17/09/2026, migração 051)

Pedido do Yan: "Não tem como alterar esses dados nem sendo admin lá dentro. Quero poder mudar sim, e quando mudar lá tem que mudar no ERP do Fábio também." Admin, financeiro, gerente e representante (na própria carteira) editam razão social, fantasia, IE, WhatsApp, e-mail, observações e endereço; CPF/CNPJ só admin e financeiro. Cada edição de cliente que já tem código fica **pendente** em `customer_changes` (051) até chegar ao Control (menos a só de WhatsApp, desde 22/09/2026 — ver 0.9): o financeiro recebe push e confirma "Já atualizei no Control", ou o próprio Control fecha a pendência pela API.

**O que muda para o Fábio — tudo aditivo, nenhum campo renomeado:** (1) no `GET /partner/v1/clientes?desde=` cada cliente ganha `alterado_no_app: { em, campos } | null`, com os campos nos nomes do contrato (`razao_social`, `nome_fantasia`, `cnpj_cpf`, `inscricao_estadual`, `email`, `observacoes`, `endereco` — **nunca `whatsapp`**, ver 0.9) — é o que ele deve aplicar no Control; (2) no `POST /partner/v1/clientes`, campo com edição pendente que chega com **outro** valor não é gravado (vem o aviso "Cliente X: campos alterados no app ainda não aplicados no Control — mantido o valor do app"), e o **mesmo** valor — o atual do app, sem diferença de `\r\n` para `\n` — fecha a pendência sozinho (`erp_atualizado_via = 'api'`); o endereço confere como grupo; o cliente sem código adotado pelo CNPJ tem conferidas do mesmo jeito as correções feitas no app antes de o código voltar (a que o registro da adoção nem traz também passa a sair em `alterado_no_app`, porque a adoção é a única conferência delas); um registro cujo cliente foi salvo no app no meio do lote volta em `ignorados` com `cadastro alterado no app durante o envio — reenvie`; com `cnpj_cpf` pendente, o registro com o documento antigo casa pelo `codigo`, e a `chave` do GET já é o documento novo (casar pelo `codigo`) — e, quando o documento foi corrigido antes de o código voltar, o registro com o documento antigo **adota** o cadastro do app em vez de criar um segundo (o mesmo no `POST /retrato`), mesmo com o documento hoje noutro cadastro — se esse outro também está sem código, a razão social do registro decide, e sem como decidir o registro volta em `ignorados` (`CNPJ corrigido no app num cliente sem código e hoje de outro cadastro sem código — confira qual dos dois é este cliente`), sem gravar em nenhum dos dois; (3) o `servidor_hora` dos `GET /clientes` e `/representantes` passa a ser a hora de **antes** da consulta menos 2 minutos de folga, e a lista é paginada pela ordem e não pela posição — usá-lo como o próximo `desde` já não pula mudança gravada durante a leitura (o que mudou nesses 2 minutos sai de novo, inofensivo); e o cliente com edição pendente sai em toda puxada até o Control ter a edição. Sem a 051, a API responde como antes (com `alterado_no_app: null`). Contrato em `docs/API-PARCEIRO.md` ("Cliente editado no app").

### 0.9 O WhatsApp do cliente é do app (22/09/2026)

Pedido do Yan: "que eu possa alterar o wtss do cliente sem ter que subir pro control, numero uma coisa numero de wtss outro". O WhatsApp do cliente passou a ser dado **só do app** — o telefone do Control é outra coisa. No app, a troca só do WhatsApp grava o histórico (`customer_changes`) mas **não fica pendente** (`erp_pendente = false`): sem push ao financeiro, sem cartão "Para atualizar no Control", sem fila da Minha área. Numa edição mista (WhatsApp e e-mail, p.ex.), a linha fica pendente pelo e-mail e o WhatsApp continua no histórico dela.

**O que muda para o Fábio — aditivo, nenhum campo renomeado:** (1) o `alterado_no_app` do `GET /partner/v1/clientes` **nunca traz `whatsapp`** (a edição mista sai só com `email`); o `whatsapp` do registro continua saindo, só para leitura; (2) no `POST /partner/v1/clientes`, o `whatsapp` **só preenche quem está sem**: cliente novo recebe o do Control, cliente existente com o WhatsApp vazio no app também; no cliente que já tem WhatsApp no app, o valor mandado (inclusive `null`) é **ignorado, sem aviso** — é a regra, não um conflito. Vale com e sem a 051. A carga por código (`_tools/importar-clientes.mjs`) segue a mesma regra; a de carteira (`importar-carteira.mjs`) já só preenchia vazio. Revisão do mesmo dia (22/09/2026): o WhatsApp que não é telefone (menos de 8 dígitos — os 11 clientes com um nome no lugar do número, gravado pela carga de carteira antiga) conta como vazio e é trocado pelo do Control; o WhatsApp que alguém **apagou no app** (há edição dele no histórico da 051) continua vazio — sem isso, o telefone do Control voltava no envio seguinte; e o `whatsapp` vazio (`null` ou `""`) nunca apaga nada. Na tabela "Quem é dono de cada dado" do contrato, o WhatsApp do cliente é do **app**.
