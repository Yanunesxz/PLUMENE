-- ═══════════════════════════════════════════════════════════════════════════
-- RODAR NO SQL EDITOR DO SUPABASE — SÓ NA CORPO SENSUAL
--
-- A PLUMENE já tem as duas coisas deste arquivo. NÃO cole lá.
--
-- Passo 1 da fase 0 (PRONTIDAO §9.1): a 013 inteira e o índice único da 042,
-- que faltam no banco da Corpo Sensual (conferido pelo Yan em 15/09/2026 por
-- pg_indexes). É idempotente: rodar duas vezes não faz mal.
--
-- Por que a 013 INTEIRA, e não só o pedaço do histórico: colar só a tabela
-- order_status_history deixaria a tabela sem a função e o gatilho que a
-- preenchem.
--
-- ANTES DE RODAR — a consulta de repetidos da 042 (deve vir VAZIA; se vier
-- alguma linha, pare e avise: o índice único recusaria o SQL inteiro):
--
...
--   • todo UPDATE em products, product_variants, product_prices, customers,
--     orders e price_tables passa a carimbar updated_at pelo banco (o CRM relê
--     mais linhas na rodada seguinte);
--   • order_status_history nasce vazia e passa a registrar cada mudança de
--     status e de faturado;
--   • as travas NOT VALID do bloco A valem para gravações novas (conferido em
--     15/09: nenhuma gravação existente seria recusada);
--   • dois pedidos da mesma empresa nunca mais com o mesmo número do Control.
--
-- DEPOIS DE RODAR, no SQL Editor:
--   SELECT count(*) FROM pg_trigger WHERE NOT tgisinternal AND tgname IN
--     ('trg_products_updated_at', 'trg_product_variants_updated_at',
--      'trg_product_prices_updated_at', 'trg_customers_updated_at',
--      'trg_orders_updated_at', 'trg_price_tables_updated_at',
--      'trg_orders_status_history');                                 -- deve dar 7
--   SELECT count(*) FROM pg_indexes
--    WHERE indexname = 'idx_orders_company_erp_order';                -- deve dar 1
-- E pela API: node _tools/conferir-013.mjs (order_status_history deve dar OK).
-- ═══════════════════════════════════════════════════════════════════════════


-- ═══ 013_protecoes.sql, inteira (copiada de apps/api/src/config/migrations) ═══
...
-- ═══ 042_numero_do_control_unico.sql: o índice único ════════════════════════
-- O número do pedido no Control é de UM pedido só, por empresa. Índice parcial:
-- pedido sem número (a imensa maioria) não colide com nada.
...
-- Recarrega o cache do Supabase: sem isto a tabela order_status_history pode
-- existir no banco e continuar invisível para a API (erro PGRST205).
